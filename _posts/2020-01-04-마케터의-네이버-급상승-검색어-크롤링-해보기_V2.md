...
그래서 단순 BeautifulSoup만이 아닌 Selenium을 함께 활용하여 네이버 급상승 검색어를 크롤링 해보는 방법을 소개해 보려 합니다. 기존에는 네이버 메인에서 데이터를 가져왔다면, 새로운 방법은 조금더 다양한 정보를 접할 수 있는 네이버의 DataLab에서 급상승 검색어를 크롤링 했습니다.
...
![Naver2-1](/images/post/Naver2/Naver_DataLab_1.PNG) 

<br>

<br>

접속을 하게되면 날짜 와 시간 및 여러가지 상세 옵션에 따른 급상승 검색어를 확인할 수 있는데, 이 페이지에서 1위부터 20위까지의 급상승 검색어를 크롤링하는 프로그램을 만들었습니다.

![Naver2-2](/images/post/Naver2/Naver_DataLab_2.PNG) 
...
![Naver2-3](/images/post/Naver2/Chrome_Driver_1.PNG)
...
![Naver2-4](/images/post/Naver2/Chrome_Driver_2.PNG)

<br>

<br>

버전을 확인 하였으니 이제 Chrome 드라이버 사이트에 접속하여 자신의 버전에 적합한 드라이버를 다운 받아 보겠습니다. Chrome 드라이버 다운로드 사이트 접속 :  <a href="https://sites.google.com/a/chromium.org/chromedriver/downloads" target="blank">Chrome Driver Downloads Site</a>

전 85버전이므로 해당 버전 다운로드 받기를 클릭했습니다.

![Naver2-5](/images/post/Naver2/Chrome_Driver_3.PNG)

<br>

<br>

다운로드 받기를 클릭하게 되면 OS 환경에 따른 압축파일들이 존재하는데, 그 중 자신에게 적합한 파일을 클릭하여 다운 받으시면 됩니다. 전 Window를 사용하고 있으므로 win32.zip 파일을 다운 받았습니다. 

![Naver2-6](/images/post/Naver2/Chrome_Driver_4.PNG)

<br>

<br>

다운 받은 파일을 압축 풀게되면 **chromedriver.exe** 파일이 있는 것을 확인하실 수 있을텐데, 이 Chrome Driver 실행 파일을 네이버 급상승 검색어 크롤링 프로그램과 동일한 공간 및 폴더에 두시면 됩니다.

![Naver2-7](/images/post/Naver2/Chrome_Driver_5.PNG)
...
총 5개의 라이브러리를 import 하였고, 각 라이브러리들의 사용 용도는 코드를 살펴보면서 하나씩 알아보도록 하하겠습니다.
...
**#2** 크롤링한 급상승 검색어 키워드는 엑셀파일에 기록 및 저장할건데, output_path는 해당 엑셀파일이 위치할 경로입니다. 전 D 드라이브를 키워드 파일이 저장될 경로로 지정했습니다.

**#3** Today는 프로그램 실행 시점의 년/월/일을 표현하기 위한 변수입니다.

**#4** order는 엑셀파일에 키워드를 저장할 때 사용할 column 값인데, 하단의 주요 코드들을 살펴보면 어렵지 않게 이해하실 수 있을겁니다.
...
**#1** 크롤링한 순위에 따른 급상승 키워드와 수집 시점의 시간을 저장하기 위한 리스트입니다.

**#2** 위에서 살펴본 chromedriver.exe의 실행 옵션을 지정해 주기 위함인데, Chrome 드라이버 사용시 백그라운드에서 Chrome창을 띄우기 위해 사용한 옵션입니다.

**#3** 사용할 chromedriver.exe의 경로와 사용할 옵션을 지정했습니다. 

<br>

**#4** chromedriver.exe를 활용하여 DataLab에 접속했습니다. 아무 옵션없이 Chrome 드라이버를 활용하여 사이트에 접속하게 될 경우 Chrome 창이 함께 띄워지지만, **#2**에서 지정한 옵션으로 인해 Chrome 창이 열리지 않고 작업이 이뤄집니다. 

다만, DataLab에 접속하게 되면 항상 하단의 이미지와 동일한 상세 옵션과 전체 연령 기준으로 접속하게 됩니다. 따라서 이번 크롤링 프로그램은 아래와 같이 정해진 옵션과 연령을 기준으로 급상승 키워드를 크롤링하게 되는점 참고 부탁드립니다.

![Naver2-8](/images/post/Naver2/Naver_DataLab_3.PNG)
...
**#6** html 코드에서 findAll을 활용하여 급상승 검색어가 기록된 시점 정보를 획득하기 위한 코드입니다.

**#7** html 코드에서 findAll을 활용하여 급상승 검색어의 순위 정보를 획득하기 위한 코드입니다.

**#8** html 코드에서 findAll을 활용하여 급상승 키워드 정보를 획득하기 위한 코드입니다.
...
다음으로 엑셀 파일에 검색 시간과 순위에 따른 급상승 검색어들을 입력하는 keyword_write_on_excel 함수에 대해서 알아보겠습니다.

**#1** 우선 지정해둔 output_path 경로에 프로그램을 실행하는 시점의 년/월/일을 기준으로 하는 엑셀 파일이 존재하는지 확인하는 코드입니다. 만약 당일 프로그램을 실행한 적이 있다면 엑셀 파일이 생성되어 있으므로, 해당 파일을 열고 추가적으로 키워드를 기록하는 작업을 수행하게 됩니다.

**#2** 급상승 검색어 정보가 저장되어 있는 파일을 확인했을 때, 첫번째 행에서 값이 비어 있는 열을 찾는 코드입니다. 만약 하단의 이미지처럼 엑셀 파일의 1행B열까지만 데이터가 있다면, 1행C열부터 새롭게 검색어 정보를 입력하게 되는 원리입니다. 

![Naver2-9](/images/post/Naver2/Naver_DataLab_4.PNG)

**#3** 비어 있는 열을 찾았다면 먼저 Rank_Keyword 리스트의 첫번째 데이터인 급상승 검색어의 시점 정보를 입력하게 됩니다.

**#4** 시점 정보 입력이 끝나면 다음 행부터는 1위부터 20위까지의 급상승 검색어를 입력하게 됩니다.

**#5** 만약 특정일에 프로그램을 실행한 적이 없어서 **#1**에서 급상승 검색어 엑셀파일을 찾지 못했다면, 새롭게 당일의 년/월/일 정보가 포함되어 있는 이름의 엑셀파일을 생성하고 저장하게 되는데, 처음 기록하는 파일이므로 1행A열부터 급상승 검색어 정보가 입력될 것입니다.
...
**#2** 성공적으로 #1의 과정이 실행된다면 datetime.today 함수를 활용해 얻은 시간과 함께 기록 완료라는 문자열을 출력합니다.
...
프로그램에 필요한 함수들을 전부 살펴봤는데, chromedriver.exe와 같은 경로에서 급상승 검색어 크롤링 코드나 프로그램을 실행하게 되면 앞서 설명한 함수들이 작동하며 급상승 검색어들이 시간대별로 분류되어 엑셀 파일에 저장되는 것을 확인할 수 있을겁니다.  

<br><br>

전체적인 코드와 실행 프로그램 다시 첨부하면서 글을 마치겠습니다.
