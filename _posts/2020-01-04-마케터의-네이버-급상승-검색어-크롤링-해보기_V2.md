...
프로그램에 필요한 함수들을 전부 살펴봤는데, chromedriver.exe와 같은 경로에서 급상승 검색어 크롤링 코드나 프로그램을 실행하게 되면 앞서 설명한 함수들이 작동하며 급상승 검색어들이 시간대별로 분류되어 엑셀 파일에 저장될 것입니다.
...
코드 및 프로그램을 chromedriver.exe와 같은 경로에 두고 실행하게 되면 D 드라이브에 상단의 이미지처럼 날짜가 포함된 이름의 엑셀 파일이 생성될 것입니다.
